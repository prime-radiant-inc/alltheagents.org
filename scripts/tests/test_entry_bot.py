...
def site_parse_frontmatter(text):
    """Parse an entry with gray-matter, the frontmatter parser Eleventy uses,
    so the writer is tested against what the site build actually reads.
    Needs node_modules (npm ci)."""
    script = "const m=require('gray-matter');let t='';process.stdin.on('data',d=>t+=d)" \
             ".on('end',()=>process.stdout.write(JSON.stringify(m(t))))"
    out = subprocess.run(["node", "-e", script], input=text, capture_output=True,
...
Counts by category are shown on the site's About page.
...
def make_repo():
    """A throwaway repo root with two entries, a ledger table, and one maker."""
    ...
    (root / "_data" / "makers.json").write_text(json.dumps({
        "zed-co": {"name": "Zed Co", "maker_type": "company", "country": "US",
                   "makes_models": False, "revenue_model": [], "website": "https://zed.example"}
    }, indent=2) + "\n")
...
class WriterTest(unittest.TestCase):
    ...
    def test_write_entry_round_trip_and_ledger(self):
        ...
        fm, body = site_parse_frontmatter(text)
        ...
        self.assertIn("Counts by category are shown on the site's About page.", ledger)
        self.assertLess(ledger.index("| `foo-agent` |"), ledger.index("| `zed-thing` |"))
        self.assertEqual(list(self.repo.makers()), ["foo-inc", "zed-co"])
        self.assertEqual(
            sorted(p.name for p in touched),
            ["CATEGORIZATION_LEDGER.md", "foo-agent.md", "makers.json"],
        )
    ...
    def test_apply_fix_touches_only_target_lines(self):
        ...
        self.assertIn("| `zed-thing` | Zed Thing | multiplexer | Runs other agents. |", self.repo.ledger_md.read_text())
        self.assertTrue(path.read_text().endswith("---\n\nNew body.\n"))
        self.assertEqual(sorted(p.name for p in touched), ["CATEGORIZATION_LEDGER.md", "alpha.md"])
