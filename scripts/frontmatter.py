...
Two writers, deliberately:
  ...
  update_record()  Surgical. Rewrites only the keys you name and leaves every
                   other byte of the file alone. This is what classification
                   work uses, so a classification commit diffs as the handful
                   of lines it actually changed.

  write_record()   Full re-emit through yaml.safe_dump. This reformats the
                   whole file, so it is only for scripts/normalize_frontmatter.py.
                   Using it for classification would rewrite all 1,164 records
                   and bury real changes in formatting noise.
"""
...
def write_record(path: Path, data: dict, body: str) -> None:
    """Full re-emit. Reformats the file -- normalization only, never classification."""
    frontmatter = yaml.safe_dump(
        data, allow_unicode=True, sort_keys=False, default_flow_style=False
    )
    Path(path).write_text(
        f"{DELIMITER}{frontmatter}{DELIMITER}\n{body.rstrip()}\n", encoding="utf-8"
    )
