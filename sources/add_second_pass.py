#!/usr/bin/env python3
"""Add second_pass_repos.txt entries into merged.tsv as new candidates."""
import os, csv
...
with open(merged, "a") as f:
    for line in open(os.path.join(SRCDIR,"second_pass_repos.txt")):
        parts = line.rstrip("\n").split("\t")
        ...
        f.write("\t".join([name,url,repo,"","",stars,desc,"","","github_topic"])+"\n")
        added += 1
